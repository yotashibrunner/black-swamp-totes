...
async function createCharge(bookingId, body, operatorId) {
  ...
  const sms = billingMethod === 'card_on_file'
    ? `Black Swamp Totes: a ${chargeTypeLabel(chargeType).toLowerCase()} charge of ${amountFmt} was applied to your card on file for rental ${booking.ref_code}. Questions? (419) 262-2837`
...
async function cancelBooking(ref) {
  ...
  if (!['paid', 'confirmed'].includes(booking.status)) {
    throw badRequest('This booking can no longer be cancelled online. Please call (419) 262-2837.', 409);
  }
  ...
  await notifyCustomer(booking, {
    sms: totalRefund > 0
      ? `Black Swamp Totes: your booking ${booking.ref_code} is cancelled. A refund of ${refundFmt} (${policyLabel}) is being processed. Questions? (419) 262-2837`
      : `Black Swamp Totes: your booking ${booking.ref_code} is cancelled. Per our policy, no refund applies. Questions? (419) 262-2837`,
